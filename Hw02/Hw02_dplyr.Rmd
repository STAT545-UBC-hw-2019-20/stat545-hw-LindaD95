...
Trying to see which country in Asia had the largest gdpPercap and which had the lowest in 2007, displayed as a tsibble. 

``` {r}
gapminder %>%  # trying to show the higest and the lowest gdppercap 
  filter(continent == 'Asia') %>%
  filter(year >= 2007) %>%
  group_by(country) %>%
  summarize(year, gdpPercap = max(gdpPercap))  
```
